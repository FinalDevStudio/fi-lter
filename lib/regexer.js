...
function split(text) {
  const selected = [];
  const parts = (text || '').replace(WHITESPACE_REGEX, SPACE)
    .split(SPLIT_REGEX);
 ...
 *
 * @param {String} str The string to escape and insensitivize.
 *
 * @returns {String} The filtered string.
 */
function filter(str) {
  const escaped = regesc(str || '').toLowerCase();
  return diregex.toString()(escaped || '');
}
