  ...
  ranges.forEach((range) => {
    if (Object.prototype.hasOwnProperty.call(params, range.name)) {
      ...
      if (is.number(num) && num > 0) {
        ...
        stage.$match[range.field] = {};
        stage.$match[range.field][range.cond] = num;
...
function byKeywords(keywords, slugStage, groupStage) {
  ...
  if (is.not.object(slugStage)) {
    throw new Error('The slugStage argument must be an Object.');
  }

  if (is.not.object(groupStage)) {
    throw new Error('The groupStage argument must be an Object.');
  }
